		            ...
		            <div class="col-12">
		                <div class="sw_no">
							...
							<div class="card_notice">
								<div class="notice_title">抗癌故事 <router-link to="notice/list">更多</router-link></div>
								<!-- 公告组件 -->
		    ...
		    <div class="container">
		        <div class="row">
		            <div class="col">
						...
						<!-- 推荐抗癌方法模块(开始) -->
		<div class="warp" v-if="$check_action('/anticancer_methods/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/anticancer_methods/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_anticancer_methods = !show_list_anticancer_methods">
		                        <div class="switch_box">
		                            <span v-if="show_list_anticancer_methods"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!-- 抗癌方法推荐列表组件 -->
		                <list_anticancer_methods v-if="show_list_anticancer_methods" :list="list_anticancer_methods" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_anticancer_methods.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_anticancer_methods" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_anticancer_methods" :key="i" @click="to_details('anticancer_methods',o,'anticancer_methods_id')">
		                            <td v-for="(oj,n) in fields_anticancer_methods">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				show_list_article: true,
								show_list_anticancer_methods: true,
				list_anticancer_methods: [],
				vm_arr_anticancer_methods: [
					"anticancer_methods_id",
							"anti_cancer_title",
							"anticancer_type",
							"cover_photo",
							"release_time",
				...
				fields_anticancer_methods: [
										{
						key: "anti_cancer_title",
						label: "抗癌标题",
						type: "文本"
					},
													{
		...
		methods: {
			...
			toToggle(){
				this.isAdmin = !this.isAdmin;
							...
							// 获取抗癌方法列表
			get_anticancer_methods() {
				let url = "~/api/anticancer_methods/get_list?";
										this.$get(url, {
									"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_anticancer_methods = json.result.list;
					}
				})
			},
		...
		computed: {
						    list_table_doctors_comments() {
		        var list = this.list_anticancer_methods;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_anticancer_methods.map((o) => {
		                // 第二个中括号是对象的属性
