<template>
	<div class="diy_details page_anticancer_methods" id="anticancer_methods_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_anticancer_methods'>
							<div_anticancer_methods :obj="obj"></div_anticancer_methods>
...
<script>
	import div_anticancer_methods from "@/components/diy/div_anticancer_methods.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "anticancer_methods_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/anticancer_methods/set?anticancer_methods_id=' + obj["anticancer_methods_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "anticancer_methods",
							source_field: "anticancer_methods_id",
							source_id: this.obj.anticancer_methods_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.anticancer_methods_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "anticancer_methods",
					source_field: "anticancer_methods_id",
					source_id: obj["anticancer_methods_id"],
					user_id
				};
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
								query.title = this.obj.anti_cancer_title
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/anticancer_methods/set?anticancer_methods_id=' + _this.obj["anticancer_methods_id"], {
								praise_len
							}, (res) => {
								if(res.result){
